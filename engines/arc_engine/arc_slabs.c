 ...
 */
static int do_slabs_newslab(struct arc_engine *engine, const unsigned int id, int force);
static void *memory_allocate(struct arc_engine *engine, size_t size);
...
unsigned int slabs_clsid(struct arc_engine *engine, const size_t size) {
 ...
 * accordingly.
 */
ENGINE_ERROR_CODE slabs_init(struct arc_engine *engine,
                             const size_t limit,
                             const double factor,
                             const bool prealloc) {
    ...
    while (++i < POWER_LARGEST && size <= engine->config.item_size_max / factor) {
        ...
        size *= factor;
		engine->items.virt_p[i] = 0;
		engine->items.sizes[ARC_LIST_B1][i] = 0;
		engine->items.sizes[ARC_LIST_B2][i] = 0;
		engine->items.sizes[ARC_LIST_T1][i] = 0;
		engine->items.sizes[ARC_LIST_T2][i] = 0;
	...
	engine->items.virt_p[engine->slabs.power_largest] = 0;
    if (engine->config.verbose > 1) {
        EXTENSION_LOGGER_DESCRIPTOR *logger;
        logger = (void*)engine->server.extension->get_extension(EXTENSION_LOGGER);
        logger->log(EXTENSION_LOG_INFO, NULL,
...
static void slabs_preallocate (const unsigned int maxslabs) {
    ...
    for (i = POWER_SMALLEST; i <= POWER_LARGEST; i++) {
        if (++prealloc > maxslabs)
            return;
        do_slabs_newslab(i, 0);
    }

}
#endif

static int grow_slab_list (struct arc_engine *engine, const unsigned int id) {
...
static int do_slabs_newslab(struct arc_engine *engine, const unsigned int id, int force) {
    ...
    if ((engine->slabs.mem_limit && engine->slabs.mem_malloced + len > engine->slabs.mem_limit && p->slabs > 0) ||
		...
		if ((!force) || (grow_slab_list(engine, id) == 0) || ((ptr = malloc((size_t)len)) == NULL)) {
...
/*@null@*/
static void *do_slabs_alloc(struct arc_engine *engine, const size_t size, unsigned int id, int force) {
    slabclass_t *p;
    void *ret = NULL;
    ...
    if (! (p->end_page_ptr != 0 || p->sl_curr != 0 ||
           do_slabs_newslab(engine, id, force) != 0)) {
        ...
        if (--p->end_page_free != 0) {
			...
			assert ((((hash_item *)p->end_page_ptr)->slabs_clsid == 0));
        } else {
            p->end_page_ptr = 0;
        }
    }
...
static void do_slabs_free(struct arc_engine *engine, void *ptr, const size_t size, unsigned int id) {
    slabclass_t *p;
    ...
    p = &engine->slabs.slabclass[id];
	assert ((((hash_item *)p->end_page_ptr) == NULL) || (((hash_item *)p->end_page_ptr)->slabs_clsid == 0));
...
/*@null@*/
static void do_slabs_stats(struct arc_engine *engine, ADD_STAT add_stats, const void *cookie) {
    int i, total;
...
void *slabs_alloc(struct arc_engine *engine, size_t size, unsigned int id, int force) {
    void *ret;

    pthread_mutex_lock(&engine->slabs.lock);
    ret = do_slabs_alloc(engine, size, id, force);
    pthread_mutex_unlock(&engine->slabs.lock);
    return ret;
}

void slabs_free(struct arc_engine *engine, void *ptr, size_t size, unsigned int id) {
    pthread_mutex_lock(&engine->slabs.lock);
    do_slabs_free(engine, ptr, size, id);
    pthread_mutex_unlock(&engine->slabs.lock);
}

void slabs_stats(struct arc_engine *engine, ADD_STAT add_stats, const void *c) {
    pthread_mutex_lock(&engine->slabs.lock);
    do_slabs_stats(engine, add_stats, c);
    pthread_mutex_unlock(&engine->slabs.lock);
}

void slabs_adjust_mem_requested(struct arc_engine *engine, unsigned int id, size_t old, size_t ntotal)
{
    pthread_mutex_lock(&engine->slabs.lock);
    slabclass_t *p;
    if (id < POWER_SMALLEST || id > engine->slabs.power_largest) {

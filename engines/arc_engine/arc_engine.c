...
static ENGINE_ERROR_CODE initalize_configuration(struct arc_engine *se,
                                                 const char *cfg_str);
...
static void set_vbucket_state(struct arc_engine *e,
...
static vbucket_state_t get_vbucket_state(struct arc_engine *e,
                                         uint16_t vbid) {
    union vbucket_info_adapter vi;
    vi.c = e->vbucket_infos[vbid];
    return vi.v.state;
...
static bool handled_vbucket(struct arc_engine *e, uint16_t vbid) {
...
ENGINE_ERROR_CODE create_instance(uint64_t interface,
   ...
   struct arc_engine *engine = malloc(sizeof(*engine));
   if (engine == NULL) {
      return ENGINE_ENOMEM;
   }

   struct arc_engine arc_engine = {
      .engine = {
         .interface = {
            .interface = 1
         },
   ...
   *engine = arc_engine;
   engine->tap_connections.clients = calloc(arc_engine.tap_connections.size, sizeof(void*));
...
static inline struct arc_engine* get_handle(ENGINE_HANDLE* handle) {
   return (struct arc_engine*)handle;
}
...
static ENGINE_ERROR_CODE default_initialize(ENGINE_HANDLE* handle,
                                            const char* config_str) {
   struct arc_engine* se = get_handle(handle);
...
static ENGINE_ERROR_CODE default_item_allocate(ENGINE_HANDLE* handle,
                                               ...
                                               const int flags,
                                               const rel_time_t exptime) {
   struct arc_engine* engine = get_handle(handle);
...
static ENGINE_ERROR_CODE default_get(ENGINE_HANDLE* handle,
   ...
   *item = item_get(engine, key, nkey);
   hash_it = get_real_item(*item);

   if (hash_it && (!(hash_it->flags & ITEM_BOTTOM))) {
      return ENGINE_SUCCESS;
   } else {
	   if (hash_it && (hash_it->flags & ITEM_BOTTOM)) {
		   hash_it->flags |= ITEM_MISSED;
		   if (hash_it->flags & ITEM_FREQUENT) {
			   if (engine->items.sizes[ARC_LIST_T1][hash_it->slabs_clsid]) {
				   //If we have a miss on a frequent item, it means that T2 is not big enough
				   int inc_value = (engine->items.sizes[ARC_LIST_T2][hash_it->slabs_clsid] / engine->items.sizes[ARC_LIST_T1][hash_it->slabs_clsid]);
				   engine->items.virt_p[hash_it->slabs_clsid] += ((inc_value > 0) ? inc_value : 1);
				   //Maximal size
				   if (engine->items.virt_p[hash_it->slabs_clsid] >
						   (engine->items.sizes[ARC_LIST_T1][hash_it->slabs_clsid] + engine->items.sizes[ARC_LIST_T2][hash_it->slabs_clsid])) {
					   engine->items.virt_p[hash_it->slabs_clsid] =
						   (engine->items.sizes[ARC_LIST_T1][hash_it->slabs_clsid] + engine->items.sizes[ARC_LIST_T2][hash_it->slabs_clsid]);
				   }
			   ...
			   if (engine->items.sizes[ARC_LIST_T2][hash_it->slabs_clsid]) {
				   //If we have a miss on a non-frequent item, it means that T1 is not big enough
				   int dec_value = (engine->items.sizes[ARC_LIST_T1][hash_it->slabs_clsid] / engine->items.sizes[ARC_LIST_T2][hash_it->slabs_clsid]);
				   engine->items.virt_p[hash_it->slabs_clsid] -= ((dec_value > 0) ? dec_value : 1);
				   //Minimal size
				   if (engine->items.virt_p[hash_it->slabs_clsid] < 0) {
					   engine->items.virt_p[hash_it->slabs_clsid] = 0;
				   }
			   } else { //prevent div by zero
				   engine->items.virt_p[hash_it->slabs_clsid] = 0;
			   }
		   }
		   item_unlink(engine, hash_it);
	   }
      return ENGINE_KEY_ENOENT;
   }
}

static void stats_vbucket(struct arc_engine *e,
                          ADD_STAT add_stat,
                          const void *cookie) {
...
static bool set_vbucket(struct arc_engine *e,
                        const void* cookie,
...
static bool get_vbucket(struct arc_engine *e,
                        const void* cookie,
...
static bool rm_vbucket(struct arc_engine *e,
                       const void *cookie,
...
static bool scrub_cmd(struct arc_engine *e,
                      const void *cookie,
                      protocol_binary_request_header *request,
                      ADD_RESPONSE response) {
...
static bool touch(struct arc_engine *e, const void *cookie,
                  protocol_binary_request_header *request,
                  ADD_RESPONSE response) {
...
static ENGINE_ERROR_CODE default_unknown_command(ENGINE_HANDLE* handle,
                                                 ...
                                                 ADD_RESPONSE response)
{
    struct arc_engine* e = get_handle(handle);
    bool sent;
...
static void default_handle_disconnect(const void *cookie,
                                      ...
                                      const void *event_data,
                                      const void *cb_data) {
    struct arc_engine *engine = (struct arc_engine*)cb_data;
    pthread_mutex_lock(&engine->tap_connections.lock);
    int ii;
    for (ii = 0; ii < engine->tap_connections.size; ++ii) {
